...
def _interactive_context(*, ignore_udf_errors: bool = False) -> KleinContext:
...
def test_take_stops_after_the_requested_limit() -> None:
    ...
    assert stream.take(5) == [{"idx": 2}, {"idx": 4}, {"idx": 6}, {"idx": 8}, {"idx": 10}]
...
def test_finite_source_take_actions_stop_at_eof(limit) -> None:
    ...
    actual = stream.take_all() if limit is None else stream.take(limit)
...
def test_map_ignores_only_failing_rows() -> None:
    context = _interactive_context(ignore_udf_errors=True)
    ...
    actual = context.from_values({"name": "Jack", "age": 3}, {"name": "Lucy", "age": 18}).map(require_adult).take_all()
...
def test_async_map_drops_only_the_failing_row() -> None:
    ...
    actual = context.from_values({"value": 1}, {"value": 2}, {"value": 3}).map(double, async_buffer_size=10).take_all()
...
def test_record_threshold_triggers_source_snapshot() -> None:
    ...
    with patch.object(LoopSourceFunction, "snapshot_state", tracked_snapshot):
        client = context.execute("debug-progress-snapshot")
        client.wait()
