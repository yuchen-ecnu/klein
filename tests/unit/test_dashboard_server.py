...
def test_dashboard_serves_packaged_page_with_strict_browser_headers(dashboard_server) -> None:
    ...
    assert b"Klein Dashboard" in payload
    assert b"Job graph" in payload
    assert b"Color uses the hottest subtask" in payload
    assert b"Scale operator" in payload

    script_status, _, script = _request(server, "GET", "/assets/dashboard.js")
    assert script_status == 200
    assert b"operator.can_rescale" in script
    assert b"rescale_disabled_reason" in script
    assert b"performanceColor" in script
    assert b"max_busy_percent" in script
...
def test_cli_dashboard_starts_bound_server(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda host, port: events.append((host, port)) or server,
    )

    result = CliRunner().invoke(
        cli.klein_cli_group,
        ["dashboard", "--host", "0.0.0.0", "--port", "8765", "--allow-unauthenticated"],
    )

    assert result.exit_code == 0
    assert "http://127.0.0.1:8765/" in result.output
    assert "WARNING" in result.output
    assert events == ["connected", ("0.0.0.0", 8765), "served", "closed"]
...
def test_cli_dashboard_refuses_unauthenticated_non_loopback_listener(monkeypatch) -> None:
    ...
    result = CliRunner().invoke(cli.klein_cli_group, ["dashboard", "--host", "0.0.0.0"])
